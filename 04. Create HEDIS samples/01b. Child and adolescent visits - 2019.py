...
denom = denom.withColumn('age_2019', 2019 - year(col('birthday')))

# Filter rows to keep only those between 3 and 20 years old
filtered_denom = denom.filter((col('age_2019') >= 3) & (col('age_2019') < 21))
...
numerator = spark.table("dua_058828_spa240.paper_4_otherservices2019_12_months")
numerator.show()
...
final_df = (final_df.withColumnRenamed('total_visits', 'total_visits_2019')
        .withColumnRenamed('any_visit', 'any_visit_2019'))
...
# Calculate the percentage of total rows for each value in 'total_visits'
total_visits_percentage_df = final.groupBy("total_visits_2019") \
                                     .agg(count("*").alias("count")) \
                                     .withColumn("percentage", round((col("count") / total_rows) * 100, 2))
...
# Calculate the percentage of total rows for each value in 'any_visit'
any_visit_percentage_df = final.groupBy("any_visit_2019") \
                                  .agg(count("*").alias("count")) \
                                  .withColumn("percentage", round((col("count") / total_rows) * 100, 2))
...
final = final.select("beneID", "state", "total_visits_2019", "any_visit_2019")
final.show()
...
final.write.saveAsTable("dua_058828_spa240.paper_4_well_child_visits_2019_12_months", mode='overwrite')
...
denom = spark.table("dua_058828_spa240.paper_4_well_child_visits_2019_12_months")
print(denom.count())
