...
outcome = spark.table("dua_058828_spa240.paper_4_lower_back_pain_02_final_12_months")
print(outcome.count())
outcome.show()
...
value_counts = df.groupBy("low_back_pass").agg(count("low_back_pass").alias("count"))
...
df = df.withColumn("outcome", when(df.low_back_pass == 1, 0).otherwise(1))
...
params_list = [
    {"colsample_bylevel": 0.75, "colsample_bytree": 0.5, "gamma": 0, "learning_rate": 0.01, "max_depth": 26, "min_child_weight": 0, "reg_alpha": 0, "reg_lambda": 0, "subsample": 0.5}
]
...
experiment_name = "/Workspace/Users/spa240@ccwdata.org/Paper 4/06. Hyperparameters - all features/02. Postpartum/02. final model/Final model - lower back - final model NO SDOH 100k"
...
best_train_predictions.write.saveAsTable("dua_058828_spa240.paper_4_lower_back_final_train_xgboost_new1_no_sdoh_new1", mode="overwrite")
best_test_predictions.write.saveAsTable("dua_058828_spa240.paper_4_lower_back_final_test_xgboost_new1_no_sdoh_new1", mode="overwrite")