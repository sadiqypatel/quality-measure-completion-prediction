...
outcome = spark.table("dua_058828_spa240.paper_4_amm_both_outcomes_months_new")
print(outcome.count())
outcome.show()
...
df = df.select("beneID","state","features","outcome2_yes")

# COMMAND ----------

# Perform value counts
value_counts = df.groupBy("outcome2_yes").agg(count("outcome2_yes").alias("count"))
...
df = df.withColumn("outcome", when(df.outcome2_yes == 1, 0).otherwise(1))
...
params_list = [
    {"colsample_bylevel": 0.5, "colsample_bytree": 0.5, "gamma": 0.0, "learning_rate": 0.01, "max_depth": 24, "min_child_weight": 0, "reg_alpha": 0, "reg_lambda": 0, "subsample": 0.25}
]
...
experiment_name = "/Workspace/Users/spa240@ccwdata.org/Paper 4/06. Hyperparameters - all features/07. AMM - Acute Phase/02. final model/Final model - AMM Contin - no sdoh - run 1"
...
best_train_predictions.write.saveAsTable("dua_058828_spa240.paper_4_cont_acute_train_xgboost_new1_no_sdoh", mode="overwrite")
best_test_predictions.write.saveAsTable("dua_058828_spa240.paper_4_cont_acute_test_xgboost_new1_no_sdoh", mode="overwrite")