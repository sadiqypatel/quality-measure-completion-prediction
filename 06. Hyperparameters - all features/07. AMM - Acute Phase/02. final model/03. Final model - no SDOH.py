...
df = df.select("beneID","state","features","outcome1_yes")

# COMMAND ----------

# Perform value counts
value_counts = df.groupBy("outcome1_yes").agg(count("outcome1_yes").alias("count"))
...
df = df.withColumn("outcome", when(df.outcome1_yes == 1, 0).otherwise(1))
...
params_list = [
    {"colsample_bylevel": 1, "colsample_bytree": 0.5, "gamma": 0.01, "learning_rate": 0.01, "max_depth": 24, "min_child_weight": 0, "reg_alpha": 0, "reg_lambda": 0, "subsample": 0.25}
]
...
experiment_name = "/Workspace/Users/spa240@ccwdata.org/Paper 4/06. Hyperparameters - all features/07. AMM - Acute Phase/02. final model/Final model - AMM Acute - no sdoh - run 1"
...
best_train_predictions.write.saveAsTable("dua_058828_spa240.paper_4_amm_acute_train_xgboost_new1_no_sdoh", mode="overwrite")
best_test_predictions.write.saveAsTable("dua_058828_spa240.paper_4_amm_acute_test_xgboost_new1_no_sdoh", mode="overwrite")